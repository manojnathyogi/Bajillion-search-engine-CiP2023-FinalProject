...
def create_index(filenames, index, file_titles):
    ...
    for file_name in filenames:
        with open(file_name) as f:
            ...
            file_titles[file_name] = first_line[:-1]
            #print(first_line[:-1]) #Debug Sentence
            for line in f: # loops over remaining lines of file
                ...
                if word_list != []:
                    for word in word_list:
                        ...
                        if term in index:
                            if file_name not in index[term]:
...
def search(index, query):
    ...
    """
    #pass
    words_separate = query.split()
    #print(words_separate) # for debug
    if len(words_separate) != 0:
        if len(words_separate) == 1:
            if words_separate[0] in index:
                return index[query]
            else:
                return []
        else:
            if words_separate[0] in index and words_separate[1] in index:
                prev_list = common(index[words_separate[0]],index[words_separate[1]])
            else:
                return []
            if len(words_separate) > 2:
                for word in words_separate[2:]:
                    if word in index:
                        new_list = common(prev_list, index[word])
                        prev_list = new_list
                    else:
                        return []
                return new_list
